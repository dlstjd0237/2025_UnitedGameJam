...
namespace BIS.Pool
{
    ...
    public class PoolManager : MonoBehaviour
    {
        ...
        private GameObject _SpawnFromPool(string tag, Vector3 position, Quaternion rotation)
        {
            ...
            {
                PoolObj pool = Array.Find(_pools, x => x.Tag == tag);
                var obj = CreateNewObject(pool.Tag, pool.Prefab.gameObject);
                ArrangePool(obj);
            }
        ...
        private void Start()
        {
            ...
            PoolListContainSO list = Resources.Load<PoolListContainSO>("PoollingList"); ;
            List<PoolObj> poolList = new List<PoolObj>();


            for (int i = 0; i < list.PoolListContainList.Count; ++i)
            {
                for (int j = 0; j < list.PoolListContainList[i].PolllingList.Count; ++j)
                {
                    poolList.Add(list.PoolListContainList[i].PolllingList[j]);
                }
            }

            _pools = poolList.ToArray();

            //미리 생성
            foreach (PoolObj pool in _pools)
            {
